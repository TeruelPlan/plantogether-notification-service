package com.plantogether.notification.grpc.client;
...
public class TripGrpcClient {
  ...
  public IsMemberResponse isMember(String tripId, String deviceId) {
    try {
      ...
      log.error(
          "IsMember gRPC call failed for trip={} device={}: {}", tripId, deviceId, e.getStatus());
      if (code == Status.Code.UNAVAILABLE || code == Status.Code.DEADLINE_EXCEEDED) {
        throw new ResponseStatusException(
            HttpStatus.SERVICE_UNAVAILABLE, "Trip membership verification temporarily unavailable");
      }
      throw new ResponseStatusException(HttpStatus.BAD_GATEWAY, "Unable to verify trip membership");
    }
  }
  ...
  public List<TripMemberProto> getTripMembers(String tripId) {
    try {
      ...
      if (code == Status.Code.UNAVAILABLE || code == Status.Code.DEADLINE_EXCEEDED) {
        throw new ResponseStatusException(
            HttpStatus.SERVICE_UNAVAILABLE, "Trip members lookup temporarily unavailable");
      }
      throw new ResponseStatusException(HttpStatus.BAD_GATEWAY, "Unable to fetch trip members");
    }
  }
